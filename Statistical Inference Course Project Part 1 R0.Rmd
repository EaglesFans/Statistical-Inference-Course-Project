...
```{r Load ggplot2 package, echo = TRUE}
library(ggplot2)
```
```{r Load knitr package, echo = TRUE}
library(knitr)
```

### Setting Seed
```{r, setting Seed, echo = TRUE}
set.seed(1)
```

### Make a simuation of sample mean with the given Parameters per instruction
```{R initial setup with the given Parameters, echo = TRUE}
lambda <- 0.2
n <- 40
sample_mean <- NULL
...
```{r 1000 averages of 40 exponentials, echo = TRUE}
for (i in 1:no_sim) {
...
```{r converting the sample_mean values to Data Frame, echo = TRUE}
...
```{r show both theoretical distribution and the sample one in one chart , echo = TRUE}
